...
The initialization script creates collections, indexes and counters, but does not seed application data by default. To load the sample data from `data/books.json` and `data/users.json` run the seed script:

- Locally (uses `.env` if present):
```bash
python scripts/seed_mongodb.py
```

- Using Docker Compose (recommended when using containers):
```bash
docker compose build app
docker compose run --rm app-seed
```

The seed operation is idempotent (it uses upsert) and will update the `book_id_counter`/`user_id_counter` to at least the maximum numeric IDs present in the data files.
...
| `MONGODB_EXPECTED_DB_COUNT` | - | (Optional) Expected number of databases for verification during init |
| `MONGODB_EXPECTED_COLLECTIONS` | - | (Optional) Comma-separated list of collections the init script should ensure exist (e.g. `books,users`) |
